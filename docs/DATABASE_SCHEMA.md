...
| pin | text | No | Login PIN (bcrypt hash) |
...
- Driver Name
- PIN (verified server-side against a bcrypt hash; no Supabase Auth account is created for drivers — see TECHNICAL_ARCHITECTURE.md Section 5)

Owner authentication uses Supabase Auth (email + password), a single Owner account for MVP.
...
Stores Proof of Delivery (POD) images. One photo per delivery for MVP.
